//Audio for Alarm
var audio = new Audio('https://assets.mixkit.co/sfx/preview/mixkit-alarm-digital-clock-beep-1200.mp3');
audio.loop = true;

var display = document.getElementById('clock');
const myList = document.querySelector('#myList');
const addAlarm = document.querySelector('.setAlarmform')
let alarmTime = null;
let alarmTimeout = null;

const alarmList = [];  // Stores all the alarms being set 
// To play alarm audio at correct time
...
//checking whether the alarm list is includes the current time, "now"
...
// function to stop playing alarm
function clearAlarm() {
    audio.pause();
    if (alarmTimeout) {
...
// removes an alarm from the unordered list and from webpage too when "Delete Alarm" is clicked
...
//and new alarm is pushed into array
    if(isNaN(newAlarm)){
        if(!alarmList.includes(newAlarm)){
            ...
            alert(`Alarm for ${newAlarm}  has  been already set.`);
        }
    } else{
        alert(" You have entered invalid Time")
    }        
})
//it is updating time every second
